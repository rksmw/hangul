...
/* ContentData
npc_greengill_slave
npc_thalorien_dawnseeker_quest_1
EndContentData */
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
	{
		...
		if (player->HasItemCount(49879, 1) || player->HasItemCount(49889, 1))
			AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Llevame a la Fuente del Sol", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
		player->SendPreparedGossip(creature);
		return true;
	}
	...
	struct npc_queldelar_sunwell_plateauAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_QUEST_STEP_1:
				...
				if (GameObject* quelDelar = me->SummonGameObject(GO_QUEL_DANAR, 1683.99f, 620.231f, 29.3599f, 0.410932f, G3D::Quat(), 90000))
				{
